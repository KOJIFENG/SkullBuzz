// Copyright (c) 2015 The SkullBuzz developers
		      ...
		      if(!fInvalid)
		      {
			    ...
			    { 
			        ...
			        if(ExtractDestination(atxout.scriptPubKey, dest))
			        {
			    	        ...
			    	        std::string skullbuzzAddress = CBitcoinAddress(dest).ToString();
				        CRichListDataItem richListDataItem;
				        richListDataItem.sAddress = skullbuzzAddress;
				        if(richListData.mapRichList.find(skullbuzzAddress) == richListData.mapRichList.end())
				            richListData.mapRichList.insert(make_pair(skullbuzzAddress, richListDataItem));
					
				        richListData.mapRichList[skullbuzzAddress].nBalance = richListData.mapRichList[skullbuzzAddress].nBalance - atxout.nValue;
