...
void Shutdown()
{
    ...
    RenameThread("skullbuzz-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: skullbuzz.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: skullbuzzd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeskullbuzzamount=<n> " + _("Keep N SkullBuzz anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SkullBuzz is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SkullBuzz is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SkullBuzz version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "SkullBuzz server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SkullBuzz") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart SkullBuzz to complete") << "\n";
    ...
    nAnonymizeSkullBuzzAmount = GetArg("-anonymizeskullbuzzamount", 0);
    if(nAnonymizeSkullBuzzAmount > 999999) nAnonymizeSkullBuzzAmount = 999999;
    if(nAnonymizeSkullBuzzAmount < 2) nAnonymizeSkullBuzzAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeSkullBuzzAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize SkullBuzz Amount %d\n", nAnonymizeSkullBuzzAmount);
    ...
    */
	...
	{
	    uiInterface.NotifySkullbuzzNodeChanged(skullbuzz.second);
	}
