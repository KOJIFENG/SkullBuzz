...
int nAnonymizeSkullBuzzAmount = 1000;
...
#else
    const char* pszModule = "skullbuzz";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SkullBuzz
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SkullBuzz
    // Mac: ~/Library/Application Support/SkullBuzz
    // Unix: ~/.skullbuzz
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SkullBuzz";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "skullbuzz.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "skullbuzzd.pid"));
