...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("skullbuzz"))
        return false;
...
{
    // Convert skullbuzz:// to skullbuzz:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("skullbuzz://"))
    {
        uri.replace(0, 12, "skullbuzz:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SkullBuzz.lnk";
}
...
{
    return GetAutostartDir() / "skullbuzz.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("SkullBuzz-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  skullbuzz-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("SkullBuzz-Qt"));
