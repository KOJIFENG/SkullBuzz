...
#include "addeditskullbuzznode.h"
#include "skullbuzznodeconfigdialog.h"
...
static void NotifySkullbuzzNodeUpdated(MasternodeManager *page, CSkullbuzzNodeConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updateSkullbuzzNode", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifySkullbuzzNodeChanged.connect(boost::bind(&NotifySkullbuzzNodeUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifySkullbuzzNodeChanged.disconnect(boost::bind(&NotifySkullbuzzNodeUpdated, this, _1));
}
...
void MasternodeManager::updateSkullbuzzNode(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_skullbuzz);
    bool bFound = false;
    int nodeRow = 0;
...
void MasternodeManager::updateNodeList()
{
    ...
    {
        LOCK(cs_skullbuzz);
        BOOST_FOREACH(PAIRTYPE(std::string, CSkullbuzzNodeConfig) skullbuzz, pwalletMain->mapMySkullbuzzNodes)
        {
            updateSkullbuzzNode(QString::fromStdString(skullbuzz.second.sAlias), QString::fromStdString(skullbuzz.second.sAddress), QString::fromStdString(skullbuzz.second.sMasternodePrivKey), QString::fromStdString(skullbuzz.second.sCollateralAddress));
        }
    }
}
...
{
    AddEditSkullbuzzNode* aenode = new AddEditSkullbuzzNode();
    aenode->exec();
}
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CSkullbuzzNodeConfig c = pwalletMain->mapMySkullbuzzNodes[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    SkullbuzzNodeConfigDialog* d = new SkullbuzzNodeConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    confirm = QMessageBox::question(this, "Delete Skullbuzz Node?", "Are you sure you want to delete this skullbuzz node configuration?", QMessageBox::Yes|QMessageBox::No);
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMySkullbuzzNodes.erase(sAddress);
        walletdb.EraseSkullbuzzNodeConfig(c.sAddress);
...
void MasternodeManager::on_startButton_clicked()
{
    ...
    QMessageBox msg;
    if(result)
        msg.setText("Skullbuzz Node at " + QString::fromStdString(c.sAddress) + " started.");
    else
...
void MasternodeManager::on_stopButton_clicked()
{
    ...
    if(result)
    {
        msg.setText("Skullbuzz Node at " + QString::fromStdString(c.sAddress) + " stopped.");
    }
    else
    {
        msg.setText("Error: " + QString::fromStdString(errorMessage));
...
void MasternodeManager::on_startAllButton_clicked()
{
    ...
    BOOST_FOREACH(PAIRTYPE(std::string, CSkullbuzzNodeConfig) skullbuzz, pwalletMain->mapMySkullbuzzNodes)
    {
        CSkullbuzzNodeConfig c = skullbuzz.second;
	std::string errorMessage;
        bool result = activeMasternode.RegisterByPubKey(c.sAddress, c.sMasternodePrivKey, c.sCollateralAddress, errorMessage);
	if(result)
	{
