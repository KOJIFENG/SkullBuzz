...
#include "ui_addeditskullbuzznode.h"
...
AddEditSkullbuzzNode::AddEditSkullbuzzNode(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::AddEditSkullbuzzNode)
{
    ui->setupUi(this);

}

AddEditSkullbuzzNode::~AddEditSkullbuzzNode()
{
    delete ui;
}


void AddEditSkullbuzzNode::on_okButton_clicked()
{
	...
	bool bForceNew = false;
        ...
        pwalletMain->mapMySkullbuzzNodes.insert(make_pair(c.sAddress, c));
	walletdb.WriteSkullbuzzNodeConfig(c.sAddress, c);
        uiInterface.NotifySkullbuzzNodeChanged(c);

        accept();
    }
}

void AddEditSkullbuzzNode::on_cancelButton_clicked()
{
    reject();
}
