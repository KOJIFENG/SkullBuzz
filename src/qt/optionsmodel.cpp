...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSkullBuzzAmount"))
        settings.setValue("nAnonymizeSkullBuzzAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeSkullBuzzAmount = settings.value("nAnonymizeSkullBuzzAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeSkullBuzzAmount"))
        SoftSetArg("-anonymizeskullbuzzamount", settings.value("nAnonymizeSkullBuzzAmount").toString().toStdString());
#ifdef USE_NATIVE_I2P
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeSkullBuzzAmount:
            nAnonymizeSkullBuzzAmount = value.toInt();
            settings.setValue("nAnonymizeSkullBuzzAmount", nAnonymizeSkullBuzzAmount);
            emit anonymizeSkullBuzzAmountChanged(nAnonymizeSkullBuzzAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
