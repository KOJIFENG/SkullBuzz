...
void ThreadIRCSeed()
{
    ...
    RenameThread("skullbuzz-ircseed");
...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #skullbuzzcoinTEST\r");
            Send(hSocket, "WHO #skullbuzzcoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #skullbuzzcoin\n");
            // randomly join #skullbuzzcoin00-#skullbuzzcoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #skullbuzzcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #skullbuzzcoin%02d\r", channel_number).c_str());
        };
